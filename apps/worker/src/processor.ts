...
export async function videoProcessor(job: Job<VideoJobData>) {
  ...
  const publishProgress = async (percent: number, status: string) => {
    // Avoid crashing if Redis isn't fully connected during unit tests
    if (
      redisPublisher.status === 'ready' ||
      redisPublisher.status === 'connecting'
    ) {
  ...
  try {
    ...
    try {
      ...
      console.error('Failed to cleanup temporary worker files:', cleanupError);
    }
  }
}
